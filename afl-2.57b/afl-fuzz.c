...
static s32 fgo_shm_id;         /* FGo: global shared memory ID */
static u64 fgo_start_time,     /* FGo: Unix start timestamp (ms) */
    fgo_pre_time = 1800000ull, /* FGo: preparation time (ms) */
    fgo_term_prob = 20ull;     /* FGo: termination probability (*precision) */
static u8 fgo_pre_flag = 0,    /* FGo: preparation flag */
    fgo_prob_flag = 0;         /* FGo: probability flag */
...
/**
 * FGo: find suitable preparation time
 */
static void fgo_find_pre_time(void)
{
  fgo_pre_time = t_x * 60 * 1000ull / 4ull;
}

/**
 * FGo: remove fgo shm ID
 */
static void fgo_remove_shm(void)
{
  shmctl(fgo_shm_id, IPC_RMID, NULL);
}

/**
 * FGo: set up shm for FGo
 */
EXP_ST void fgo_setup_shm(void)
{
  u8 *fgo_shm_str;

  /* Allocate some bytes for instrumented program */
  fgo_shm_id = shmget(FGO_SHM_KEY, 2 * sizeof(u64), IPC_CREAT | 0666);
  if (fgo_shm_id < 0)
    PFATAL("shmget() failed at fgo_setup_shm()");

  /* Remove shm when exiting */
  atexit(fgo_remove_shm);

  /* shm value and env var */
  fgo_shm_str = alloc_printf("%d", fgo_shm_id);
  setenv(FGO_SHM_ENV_VAR, fgo_shm_str, 1);
  ck_free(fgo_shm_str);

  /* pointer to shm */
  u8 *fgo_shm_ptr = shmat(fgo_shm_id, NULL, 0);
  if (!fgo_shm_ptr)
    PFATAL("shmat() failed at fgo_setup_shm()");

  /* write something to shm */
  *((u64 *)fgo_shm_ptr) = (fgo_start_time + fgo_pre_time);
  *((u64 *)(fgo_shm_ptr + sizeof(u64))) = fgo_term_prob;
}
...
static void usage(u8 *argv0)
{
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -P prob       - basic probability of early termination (scale to 0~%u)\n\n"

       "For additional tips, please consult %s/README.\n\n",

       argv0, EXEC_TIMEOUT, MEM_LIMIT, FGO_TERM_PRECISION, doc_path);
...
int main(int argc, char **argv)
{
  ...
  /* FGo: additional options '-e' and '-P' */
#if AFLGO_IMPL
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qz:e:P:c:")) > 0)
#else
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:e:P:Q")) > 0)
#endif // AFLGO_IMPL

    switch (opt)
    {

    /* FGo: option for preparation time */
    case 'e':
    {
      if (fgo_pre_flag)
      ...
      {
      case 's':
        fgo_pre_time = (u64)temp_pre_time * 1000ull;
        break;
      case 'm':
        fgo_pre_time = (u64)temp_pre_time * 60ull * 1000ull;
        break;
      case 'h':
    ...
    /* FGo: option for termination probability */
    case 'P':
    {
      if (fgo_prob_flag)
        FATAL("Multiple -P options not supported");

      u32 temp_term_prob = 0u;
      if (sscanf(optarg, "%u", &temp_term_prob) < 1 || optarg[0] == '-')
        FATAL("Bad syntax used for -P");

      if (temp_term_prob <= 0 || temp_term_prob >= FGO_TERM_PRECISION)
        FATAL("Unsupported probability value. It should be in the range (0, %u)", FGO_TERM_PRECISION);

      fgo_term_prob = (u64)temp_term_prob;
  ...
  OKF("FGo: cut-the-loss probability is equal to " cBRI "%llu/%u" cRST "", fgo_term_prob, FGO_TERM_PRECISION);
  ...
  /* FGo: prerequisites */
  fgo_start_time = start_time;
  if (!fgo_pre_flag)
    fgo_find_pre_time();
  /* FGo: set up shared memory */
  fgo_setup_shm();
