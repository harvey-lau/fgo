...
def exec_distance_prog(
    dot, targets, out, names, cg_distance=None, cg_callsites=None, py_version=False
):
    """
    Args:
    ...
    """
    prog = DIST_PY
    cmd = [prog, "-d", dot, "-t", targets, "-o", out, "-n", names]
    if cg_distance is not None and cg_callsites is not None:
        cmd.extend(["-c", cg_distance, "-s", cg_callsites])
...
def calculating_distances(args):
    ...
    bbtargets = args.temporary_directory / "BBtargets-new.txt"
    ...
    print(
        f"({STEP}) Computing distance for control-flow graphs (this might "
        "take a while)"
    )
    with ThreadPoolExecutor(max_workers=mp.cpu_count()) as executor:
        results = executor.map(
            calculate_cfg_distance_from_file, dot_files.glob("cfg.*.dot")
        )

    try:
        for r in results:
            pass  # forward Exceptions
    except subprocess.CalledProcessError as err:
        log_p = args.temporary_directory / f"step{STEP}.log"
        with log_p.open("w") as f:
            f.write(err.stderr.decode())
    ...
    merge_distance_files(dot_files, args.temporary_directory / "distance.cfg.txt")
    next_step(args)
...
def main():
    ...
    parser.add_argument(
        "fuzzer_name", metavar="fuzzer-name", nargs="?", help="Name of fuzzer binary"
    )
    parser.add_argument(
        "-p",
        "--python-only",
    ...
    if len(binaries) == 0:
        parser.error(
            "Couldn't find any binaries in folder " f"{args.binaries_directory}."
        )
    if args.fuzzer_name:
        ...
        if len(list(tmp)) == 0:
            parser.error(
                f"Couldn't find bytecode for fuzzer {args.fuzzer_name} "
