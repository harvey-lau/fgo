...
static u64 fgo_start_time,     /* FGo: Unix start timestamp (ms) */
    ...
    fgo_term_prob = 20ull;     /* FGo: termination probability (*100) */
...
EXP_ST void fgo_setup_shm(void)
{
  ...
  fgo_shm_id = shmget((key_t)233, 2 * sizeof(u64), IPC_CREAT | 0666);
  if (fgo_shm_id < 0)
...
int main(int argc, char **argv)
{
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:Qz:c:p:P:")) > 0)
    ...
    case 'P':
    {
      ...
      if (temp_term_prob <= 0 || temp_term_prob >= 100)
        FATAL("Unsupported probability value. It should be in the range (0, 100)");
    ...
    case 'c':
    { /* cut-off time for cooling schedule */
