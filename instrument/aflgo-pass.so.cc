  ...
  else
  {
    ...
    for (auto &F : M)
    {
      ...
      for (auto &BB : F)
      {
        ...
        {
          // FGo: call __fgo_early_term().
          FunctionCallee __fgo_early_term = M.getOrInsertFunction(
              "__fgo_early_term", Type::getVoidTy(C));

          if (F.getName() != "__fgo_early_term")
            IRB.CreateCall(__fgo_early_term, {});
        }

        /* FGo: Debug: print bb name and distance */
        auto *TI = BB.getTerminator();
        IRBuilder<> Builder(TI);

        Value *bbnameVal = Builder.CreateGlobalStringPtr(fgo_bb_name);
        Value *distVal = ConstantInt::get(Type::getInt32Ty(M.getContext()), distance, true);
        Type *Args[] = {
            Type::getInt8PtrTy(M.getContext()), // uint8_t* bb_name
            Type::getInt32Ty(M.getContext())};
        FunctionType *FTy = FunctionType::get(Type::getVoidTy(M.getContext()), Args, false);
        FunctionCallee instrumented = M.getOrInsertFunction("__fgo_debug_out_bb", FTy);
        Builder.CreateCall(instrumented, {bbnameVal, distVal});
